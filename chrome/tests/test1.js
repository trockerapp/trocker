...
function runTinyImageTests(images, resBox, correctAns) {
	...
	for (var i = 0; i < images.length; i++) {
		...
		var failColor = img.parentElement.getAttribute('failColor')
			? img.parentElement.getAttribute('failColor')
			: '#e55353';
		if ((isTiny(img) || isSusp(img)) == correctAns)
			resBox.innerHTML +=
		...
		if (showDetails()) {
			resBox.innerHTML +=
				'<div>HTML: <textarea disabled style="width:100%">' + img.outerHTML + '</textarea></div>';
			var s = getSize(img);
		...
		for (var j = 0; j < samples.length; j++) {
			if (name == samples[j].name) {
				...
				for (var url of thisSamples.samples.hit) {
					...
					if (hitCnt > 0) {
						// At least one pattern matches the url
						// Ok
						passCount++;
					} else {
						hitFailCount++;
				...
				for (var url of thisSamples.samples.miss) {
					...
					hitCnt += checkPatternMatches(ot.patterns, url);
					if (hitCnt == 0) {
						// None of the patterns matche the url
						// Ok
						passCount++;
					} else {
						missFailCount++;
